...
namespace ICUDashboard.Tests
{
    public abstract class ICUDashboardTestBase : AbpIntegratedTestBase<ICUDashboardTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ICUDashboardTestBase()
        {
        ...
        protected void UsingDbContext(Action<ICUDashboardDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ICUDashboardDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ICUDashboardDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ICUDashboardDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ICUDashboardDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ICUDashboardDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ICUDashboardDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ICUDashboardDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ICUDashboardDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ICUDashboardDbContext, Task<T>> func)
        {
            T result;
